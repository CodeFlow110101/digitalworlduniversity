document.addEventListener("livewire:init", () => {
  Livewire.on("send-message", event => {
    ...
    if (event.file.files[0].size >= event.fileSizeLimit * 1024 * 1024) {
      Livewire.dispatch(event.callbackDispatch, {
        ...
        validationMessage: {
          file:
            "The file size should be less than " + event.fileSizeLimit + " MB."
        },
        fileName: null,
        filePath: null
      });
  ...
  Livewire.on("upload-video", event => {
    ...
    if (event.video.files.length == 0 || event.thumbnail.files.length == 0) {
      Livewire.dispatch(event.callbackDispatch, {
        validationKey: {
          video: event.video.files.length == 0,
          thumbnail: event.thumbnail.files.length == 0
        },
        validationMessage: {
          video: "The Video is required.",
    ...
    if (
      event.video.files[0].size >= event.videoSizeLimit * 1024 * 1024 ||
      event.thumbnail.files[0].size >= event.thumbnailSizeLimit * 1024 * 1024
    ) {
      Livewire.dispatch(event.callbackDispatch, {
        validationKey: {
          video:
            event.video.files[0].size >= event.videoSizeLimit * 1024 * 1024,
          thumbnail:
            event.thumbnail.files[0].size >=
            event.thumbnailSizeLimit * 1024 * 1024
        },
        validationMessage: {
          video:
            "The Video size should be less than " + event.videoSizeLimit + ".",
          thumbnail:
            "The Thumbnail size should be less than " +
            event.thumbnailSizeLimit +
            "."
        },
        videoName: null,
        videoPath: null,
    ...
    $.ajax({
      ...
      success: function(response) {
        Livewire.dispatch(event.callbackDispatch, {
          ...
          thumbnailName: response["thumbnailName"],
  ...
  Livewire.on("upload-thumbnail", event => {
    ...
    if (event.thumbnail.files.length == 0) {
      Livewire.dispatch(event.callbackDispatch, {
        validationKey: { thumbnail: event.thumbnail.files.length == 0 },
        validationMessage: { thumbnail: "The Thumbnail is required." },
        thumbnailName: null,
        thumbnailPath: null
      });
      return;
