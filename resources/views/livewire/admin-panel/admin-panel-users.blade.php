...
<div class="text-[#131e30] dark:text-[#DDE6ED] bg-[#d6dcde] dark:bg-gray-800 rounded-2xl p-6">
    <button wire:click="$dispatch('show-modal', { modal:'modal-user', args:null, data:null, callback_event:null })" class="fixed z-10 bottom-12 right-12 bg-[#d6dcde] dark:bg-gray-800 text-xl flex justify-between items-center gap-4 rounded-lg py-2 px-4">
        <div>
            ...
            <tr wire:key="user-{{$user->id}}" class="text-center text-xl sm:text-md font-semibold">
                ...
                <td x-data="{showDropdown:false}" @click.away="showDropdown=false" class="flex justify-center py-4 relative">
                    <div @click="showDropdown=!showDropdown" class="rounded-full p-2 hover:bg-gray-400 cursor-pointer">
                        <svg class="w-6 h-6 text-[#131e30] dark:text-[#DDE6ED]" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                            <path stroke="currentColor" stroke-linecap="round" stroke-width="3" d="M12 6h.01M12 12h.01M12 18h.01" />
                        </svg>
                    </div>
                    <div :class="showDropdown?'sclae-y-100':'scale-y-0'" class="absolute text-md transition-transform duration-100 rounded-xl grid grid-cols-1 gap-2 w-3/4 p-2 z-10 origin-top top-16 bg-gray-400 dark:text-[#131e30]">
