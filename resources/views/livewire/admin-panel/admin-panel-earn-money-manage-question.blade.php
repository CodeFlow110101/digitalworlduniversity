...
<div class="grow p-6 flex flex-col gap-2">
    <div class="flex justify-between">
        <div>Total Questions: {{count($survey->questions)}}</div>
        <button wire:click="toggleModal" class="border border-black rounded-md py-1 px-2">Add</button>
    </div>
    <div class="grow relative" x-data="{ height: 0 }" x-resize="height = $height">
        <div class="absolute inset-0 overflow-y-auto flex flex-col gap-4" :style="'height: ' + height + 'px;'">
            @foreach($survey->questions as $question)
            <div class="border border-black rounded-md p-4 flex flex-col gap-2">
    ...
    @if($modal)
    <div class="fixed inset-0 flex flex-col justify-center items-center border border-black">
        <form wire:submit="submit" class="w-1/2 bg-white shadow-md p-4">
            <div class="rounded-md p-4 flex flex-col gap-4">
                <div class="flex justify-between items-center">
                    <div>Question</div>
                    <div>
                        <button wire:click="toggleModal" type="button" class="hover:bg-black rounded-full p-1 group">
                ...
                <div>
                    <input wire:model="question" class="border border-black p-2 w-full outline-none rounded-md">
                    @error('question')
                ...
                <div class="flex flex-col gap-4">
                    <div class="flex items-center gap-4">
                        ...
                        <input wire:model="options_1" class="border border-black p-2 w-full outline-none rounded-md" placeholder="Option 1">
                    </div>
                    @error('options_1')
                    <div class="text-sm text-red-700">{{$message}}</div>
                    @enderror
                    <div class="flex items-center gap-4">
                        ...
                        <input wire:model="options_2" class="border border-black p-2 w-full outline-none rounded-md" placeholder="Option 2">
                    </div>
                    @error('options_2')
                    <div class="text-sm text-red-700">{{$message}}</div>
                    @enderror
                    <div class="flex items-center gap-4">
                        ...
                        <input wire:model="options_3" class="border border-black p-2 w-full outline-none rounded-md" placeholder="Option 3">
                    </div>
                    @error('options_3')
                    <div class="text-sm text-red-700">{{$message}}</div>
                    @enderror
                    <div class="flex items-center gap-4">
                        ...
                        <input wire:model="options_4" class="border border-black p-2 w-full outline-none rounded-md" placeholder="Option 4">
                    </div>
                    @error('options_4')
                    <div class="text-sm text-red-700">{{$message}}</div>
                    @enderror
            ...
            <div class="flex justify-center items-center">
                <button wire:loading.class="pointer-events-none" wire:target="submit" type="submit" class="bg-black text-white rounded-md py-1 px-2 mx-auto">Submit</button>
            </div>
        </form>
    </div>
    @endif
