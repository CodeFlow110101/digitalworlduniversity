...
on([
    'admin-panel-store-handle-file' => function ($validationKey, $validationMessage, $thumbnailName, $thumbnailPath) {
        ...
        if ($validationKey && $validationKey['thumbnail']) {
            ...
            $this->dispatch('reset-admin-panel-store');
        }
    }
]);
...
<div>
    <div class="w-full h-min grid grid-cols-1 gap-6">
        ...
        <div x-data="{showLoader:false}" x-on:admin-panel-modal-store-loader.window="showLoader = event.detail.value" class="flex justify-center w-full">
            <button :class="showLoader && 'pointer-events-none'" wire:click="$dispatch('upload-thumbnail', { thumbnail: $refs.thumbnail, thumbnailSizeLimit:1, callbackDispatch:'admin-panel-store-handle-file', callbackLoaderDispatch:'admin-panel-modal-store-loader'})" class="bg-[#131e30] px-8 py-4 text-lg font-semibold rounded-lg text-[#d6dcde]">
