...
rules(['title' => 'required|min:6', 'description' => 'required|min:6', 'thumbnail' => 'required', 'url' => 'required']);
...
on([
    'modal-job-handle-file' => function ($validationKey, $validationMessage, $thumbnailName, $thumbnailPath) {
        ...
        if ($validationKey && $validationKey['thumbnail']) {
            ...
            if (!$this->url) {
                $this->addError('url', 'The url field is required.');
            }

            $this->dispatch('modal-job-loader', value: false);
        } else {
            Job::create([
                'title' => $this->title,
...
<div>
    <div class="w-full h-min grid grid-cols-1 gap-6">
        ...
        <div>
            <div class="relative">
                <input wire:model="url" type="text" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-[#131e30] dark:text-[#DDE6ED] bg-transparent rounded-lg border-2 border-[#131e30] dark:border-[#DDE6ED] appearance-none focus:outline-none focus:ring-0 peer" placeholder=" " />
                <label for="floating_outlined" class="absolute text-sm text-[#131e30] dark:text-[#DDE6ED] duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-[#d6dcde] dark:bg-gray-800 px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Form Link</label>
            </div>
            @error('url')<div class="text-red-600">{{$message}}</div>@enderror
        </div>
        <div x-data="{showLoader:false}" x-on:modal-job-loader.window="showLoader = event.detail.value" class="flex justify-center w-full">
            <button :class="showLoader && 'pointer-events-none'" wire:click="$dispatch('upload-thumbnail', { thumbnail: $refs.thumbnail, thumbnailSizeLimit:1, callbackDispatch:'modal-job-handle-file', callbackLoaderDispatch:'modal-job-loader'})" class="bg-[#131e30] dark:bg-[#DDE6ED] px-8 py-4 text-lg font-semibold rounded-lg text-[#d6dcde] dark:text-[#131e30]">
