...
<div>
    <div class="w-full h-min grid grid-cols-1 gap-6">
        ...
        <div>
            <div class="relative">
                <input x-ref="thumbnail" type="file" accept="image/*" class="block px-2.5 pb-2.5 pt-4 w-full text-sm text-[#131e30] bg-transparent rounded-lg border-2 border-[#131e30] appearance-none focus:outline-none focus:ring-0 peer" placeholder=" " />
        ...
        <div x-data="{showLoader:false}" x-on:modal-job-loader.window="showLoader = event.detail.value" class="flex justify-center w-full">
            <button :class="showLoader && 'pointer-events-none'" wire:click="$dispatch('upload-thumbnail', { thumbnail: $refs.thumbnail, thumbnailSizeLimit:1, callbackDispatch:'modal-job-handle-file', callbackLoaderDispatch:'modal-job-loader'})" class="bg-[#131e30] px-8 py-4 text-lg font-semibold rounded-lg text-[#d6dcde]">
