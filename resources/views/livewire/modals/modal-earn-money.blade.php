...
on([
    'admin-panel-earn-money-handle-file' => function ($validationKey, $validationMessage, $thumbnailName, $thumbnailPath, $thumbnailUrl) {
        ...
        if ($validationKey && $validationKey['thumbnail']) {
            ...
            $this->dispatch('reset-admin-panel-earn-money');
        }
    }
]);
    ...
    <div class="w-full h-min grid grid-cols-1 gap-6">
        ...
        <div>
            <div class="relative">
                ...
                <label for="floating_outlined" class="absolute text-sm text-[#131e30] dark:text-[#DDE6ED] duration-300 transform -translate-y-4 scale-75 top-2 z-10 origin-[0] bg-[#d6dcde] dark:bg-gray-800 px-2 peer-focus:px-2 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Url</label>
            </div>
        ...
        <div x-data="{showLoader:false}" x-on:admin-panel-modal-earn-money-loader.window="showLoader = event.detail.value" class="flex justify-center w-full">
            <button :class="showLoader && 'pointer-events-none'" wire:click="$dispatch('upload-thumbnail', { thumbnail: $refs.thumbnail, thumbnailSizeLimit:1, callbackDispatch:'admin-panel-earn-money-handle-file', callbackLoaderDispatch:'admin-panel-modal-earn-money-loader'})" class="bg-[#131e30] dark:bg-[#DDE6ED] px-8 py-4 text-lg font-semibold rounded-lg text-[#d6dcde] dark:text-[#131e30]">
