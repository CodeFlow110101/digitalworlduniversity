...
with(fn() => [
    ...
    'video_progression' => VideoProgress::where('user_id', $this->user->id)->distinct()->pluck('program_id')->toArray(),
]);
...
<div x-data="chat()" x-init="chats = Object.values({{$chats}}); user = {{$user}}; groupId = {{ $this->group ? $this->group->id : 0}}; init();" class="grow flex flex-col gap-2 p-2 lg:p-4">
    <div class="flex justify-end lg:hidden">
        <div x-show="!showChats" @click="showChats = !showChats;" class="border border-black dark:border-white rounded-md p-1">
    ...
    <div class="text-white grow flex justify-between gap-1 lg:gap-4">
        <div class="w-1/4 lg:w-[10%] rounded-lg border dark:border-white border-black pr-2 py-2 flex flex-col">
            <div class="grow overflow-y-auto relative gap-4 my-auto rounded-lg" x-data="{ height: 0 }" x-resize="height = $height">
                <div :style="'height: ' + height + 'px;'" class="absolute inset-x-0 flex flex-col gap-4">
                    ...
                    <div wire:click="selectChannel({{$channel->id}})" class="flex justify-between items-center gap-1 w-full @if(!(Gate::check('is_Admin') || is_null($channel->program_id) || in_array($channel->program_id, $video_progression))) pointer-events-none @endif">
                        <div class="@if($channel->id == $this->channel->id) h-5/6 lg:h-1/2 @else h-4 @endif w-0 border-2 rounded-r-full dark:border-white border-black"></div>
                        <div class="w-full relative">
                            <img class="w-full aspect-square rounded-md" src="{{ $channel->thumbnail_url }}">
                            @if(!(Gate::check('is_Admin') || is_null($channel->program_id) || in_array($channel->program_id, $video_progression)))
                            <div class="absolute inset-0 bg-black/30 rounded-md flex justify-center items-center">
                                <svg class="size-3/4 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="0.4" d="M9 11V7a3 3 0 1 1 6 0v4m-8 0h10a1 1 0 0 1 1 1v7a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1v-7a1 1 0 0 1 1-1Z" />
                                </svg>
                            </div>
                            @endif
                        </div>
        ...
        <div class="size-full flex justify-between gap-4">
            <div :class="showChats || 'max-lg:hidden'" class="w-full lg:w-4/12 p-2 flex flex-col justify-around border dark:border-white border-black rounded-lg gap-4">
            ...
            <div :class="showChats && 'max-lg:hidden'" class="w-full rounded-tr-lg rounded-br-lg overflow-hidden flex flex-col border dark:border-white border-black rounded-lg p-2">
                <div class="bg-black dark:bg-white text-white dark:text-black rounded-lg text-center py-2 align-middle"># | {{ $this->group ? $this->group->name : '' }}</div>
                ...
                <div>
                    @if($group && (Gate::check('is_Admin') || is_null($channel->program_id) || in_array($channel->program_id, $video_progression)))
                    <form wire:submit="send" class="flex justify-between items-center gap-1">
